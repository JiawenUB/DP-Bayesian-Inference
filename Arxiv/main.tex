...
\caption{\footnotesize{Sensitivity of $\hellinger$}}
...
\label{subsec_effi}
The formula for computing the local sensitivity presented in Sec. \ref{subsec_emls}:$\max\limits_{\{|x,y'| \leq 1; y'\in \mathcal{X}^n\}}\max\limits_{\{r\in R\}} \{\hellinger(\bysinfer(x), r) - \hellinger(\bysinfer(y'), r)|\}$ can be reduced to $\max\limits_{\{|x,y'| \leq 1;y'\in \mathcal{X}^n\}}\hellinger(\bysinfer(x), \bysinfer(y'))|$ by applying the distance triangle property. i.e., the maximum value over $\max\limits_{r \in R}$ always achieves at $r = \bysinfer(x)$ itself, where $\Delta_{l}u(x) = \max\limits_{\{|x,y'| \leq 1;y'\in \mathcal{X}^n\}} \{\hellinger(\bysinfer(x), \bysinfer(x)) - \hellinger(\bysinfer(y'), \bysinfer(x))|\} = \max\limits_{\{|x,y'| \leq 1;y'\in \mathcal{X}^n\}} \{\hellinger(\bysinfer(y'), \bysinfer(x))|\}$. We also have some experiments for validating our proposal as in Fig. \ref{fig_efficiency}, where we calculate the $\max\limits_{\{|x,y'| \leq 1;y'\in \mathcal{X}^n\}}$ value for every candidate $r \in R$. It is shown that maximum value taken when  $r = \bysinfer(x)$.
...
\begin{center}
\centering
    \includegraphics[width=0.48\textwidth]{sampling_2d.eps}
...
\begin{center}
\centering
     \includegraphics[width=0.48\textwidth]{sampling_3d.eps}   
...
\begin{center}
\centering
     \includegraphics[width=0.48\textwidth]{sampling_4d.eps}    
...
In Figures \ref{fig_vs_datasize_2d}, \ref{fig_vs_datasize_3d} and \ref{fig_vs_datasize_4d} we consider \emph{balanced} data sets
of observations. This means that in the Beta-Binomial setting the datasets will consist of 50\% 1s and the rest 0s, while for the
Dirichelet-Multinomial (Figure  \ref{fig_vs_datasize_3d} and \ref{fig_vs_datasize_4d})
the data will be split in the $k=3$ bins with perecentages of: 33\%, 33\% and 34\% in 3 dimensionality and 25\%, 25\%, 25\% and 25\% in 4 dimensionality.
The results show that when the data size increases, the average errors of
