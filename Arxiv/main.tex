...
In what follows, let $ \ux{r} = \hellinger(\bysinfer(\dataobs), r)$ for $r \in \candidateset{\dataobs}$, we first introduce Lemma \ref{lem_score_pro_convert} and \ref{lem_cadinality}.
...
\begin{itemize}
  ...
  & \implies Pr[\hexpmech(\dataobs) = r'] = Pr[\hexpmech(\dataobs) = r''] \\
...
\label{def_cardinality}
  ...
  Define $\cardinality{r}$ as the cardinality of $R_r$ s.t. $ R_r = \{ r' : \ux{r'} = \ux{r}, r' \in \candidateset{\dataobs}\} \subset \candidateset{\dataobs}$ and $\forall \hat{r} \in \candidateset{\dataobs}, \hat{r} \notin R_r \implies \ux{\hat{r}} \neq \ux{r}$, 
\end{definition}

\begin{lem}
\label{lem_cadinality}
...
\hexpmechPr{\dataobs}{\ux{z} = \ux{r}} = \cardinality{r}\hexpmechPr{\dataobs}{z = r}
 \end{equation*}
\end{lem}
\begin{proof} of Lemma \ref{lem_cadinality}.\\
...
Apply Lemma \ref{lem_score_pro_convert} in $R_r$, \\
...
\leq
e^{(\frac{\epsilon k}{S(x)} + \ln 2)} \hexpmechPr{\dataobs}{\ux{z} = ( \ux{r} + k)}.
\end{equation*}
\end{lem}
...
\begin{itemize}
  ...
  By Lemma \ref{lem_score_pro_convert}, $\hexpmechPr{\dataobs}{\ux{z} = \ux{r}} = \cardinality{r} \hexpmechPr{\dataobs}{z = r}$.\\
  By $\ux{r} \neq 0 $, $\cardinality{r} = 2$.\\
  $\implies$\\
    \begin{equation*}
    \begin{split}
    LHS 
    & = 2 \cdot \frac{exp(\frac{\epsilon \hat{s}}{2 S(\dataobs)})}{\nomalizer{\dataobs}}\\
    & = 2 \cdot \frac{exp(\frac{\epsilon (\ux{r} + k - k)}{2 S(\dataobs)})}{\nomalizer{\dataobs}}\\
    & = 2 \cdot \frac{exp(\frac{\epsilon (\ux{r} + k)}{2 S(\dataobs)} + \frac{- \epsilon k}{2 S(\dataobs)})}{\nomalizer{\dataobs}}\\
    & = 2 \cdot \frac{exp(\frac{\epsilon (\ux{r} + k)}{2 S(\dataobs)})}{\nomalizer{\dataobs}} \cdot e^{\frac{- \epsilon k}{2 S(\dataobs)}}\\
    & = 2 \cdot \frac{exp(\frac{\epsilon (\ux{t})}{2 S(\dataobs)})}{\nomalizer{\dataobs}} \cdot e^{\frac{- \epsilon k}{2 S(\dataobs)}}\\
    & = e^{\frac{- \epsilon k}{2 S(\dataobs)}} \cdot 2 \hexpmechPr{\dataobs}{z = t}\\
    & = e^{\frac{- \epsilon k}{2 S(\dataobs)}} \cdot \cardinality{t} \hexpmechPr{\dataobs}{z = t}\\
    & = e^{\frac{- \epsilon k}{2 S(\dataobs)}} \cdot \hexpmechPr{\dataobs}{\ux{z} = \ux{t}} \\
    & = e^{\frac{- \epsilon k}{2 S(\dataobs)}} \cdot \hexpmechPr{\dataobs}{\ux{z} = (\ux{r} + k)}\\
    & \leq e^{(\frac{\epsilon k}{S(x)} + \ln 2)} \hexpmechPr{\dataobs}{\ux{z} = ( \ux{r} + k)}\\
    & = RHS.
    \end{split}
    \end{equation*}
  ...
  \begin{itemize}
    ...
    \begin{split}
    LHS 
    & = \frac{exp(\frac{\epsilon \hat{s}}{2 S(\dataobs)})}{\nomalizer{\dataobs}}\\
    & = \frac{exp(\frac{\epsilon (\ux{r} + k - k)}{2 S(\dataobs)})}{\nomalizer{\dataobs}}\\
    & = \frac{exp(\frac{\epsilon (\ux{r} + k)}{2 S(\dataobs)} + \frac{- \epsilon k}{2 S(\dataobs)})}{\nomalizer{\dataobs}}\\
    & = \frac{exp(\frac{\epsilon (\ux{r} + k)}{2 S(\dataobs)})}{\nomalizer{\dataobs}} \cdot e^{\frac{- \epsilon k}{2 S(\dataobs)}}\\
    & = \frac{exp(\frac{\epsilon (\ux{t})}{2 S(\dataobs)})}{\nomalizer{\dataobs}} \cdot e^{\frac{- \epsilon k}{2 S(\dataobs)}}\\
    & = e^{\frac{- \epsilon k}{2 S(\dataobs)}} \cdot \hexpmechPr{\dataobs}{z = t}\\
  ...
  \item $\ux{r} \neq 0, \ux{r} + k = 0$
    ...
    & = 2 \cdot \frac{exp(\frac{\epsilon (\ux{r} + k)}{2 S(\dataobs)})}{\nomalizer{\dataobs}} \cdot e^{\frac{- \epsilon k}{2 S(\dataobs)}}\\
    & = 2 \cdot \frac{exp(\frac{\epsilon (\ux{t})}{2 S(\dataobs)})}{\nomalizer{\dataobs}} \cdot e^{\frac{- \epsilon k}{2 S(\dataobs)}}\\
    & = 2 \cdot e^{\frac{- \epsilon k}{2 S(\dataobs)}} \cdot 1 \cdot \hexpmechPr{\dataobs}{z = t}\\
    & = 2 \cdot e^{\frac{- \epsilon k}{2 S(\dataobs)}} \cdot \cardinality{t} \hexpmechPr{\dataobs}{z = t}\\
    & = 2 \cdot e^{\frac{- \epsilon k}{2 S(\dataobs)}} \cdot \hexpmechPr{\dataobs}{\ux{z} = \ux{t}} \\
    & = 2 \cdot e^{\frac{- \epsilon k}{2 S(\dataobs)}} \cdot \hexpmechPr{\dataobs}{\ux{z} = (\ux{r} + k)}\\
...
\begin{lem}
\label{lem_dilation}
for any exponential mechanism $\hexpmech(\dataobs)$, $\lambda < |\beta|$, $\epsilon$, $|\delta'| < 1$ and $\beta \leq \ln(1 - \frac{\epsilon}{2 \ln (\frac{\delta'}{2 |\candidateset{\dataobs}|})})$, the dilation property holds:

\begin{equation*}
\hexpmechPr{\dataobs}{u(z,\dataobs) = c}
\leq
e^{\frac{\epsilon}{2}} \hexpmechPr{\dataobs}{u(z,\dataobs) = e^{\lambda} c} + \frac{\delta'}{2},
\end{equation*}
...
\textbf{Detail proof:}
	To show:
	\begin{equation*}
	\hexpmechPr{\dataobs}{u(z,\dataobs) = u(r,\dataobs)}
	\leq
	e^{\frac{\epsilon}{2}} \hexpmechPr{\dataobs}{u(z,x) = e^{\lambda} u(r,\dataobs)} + \frac{\delta'}{2},
	\end{equation*}
  we need to show:
  \begin{equation*}
...
\begin{itemize}
	...
	\item $\lambda > 0$
		...
		Particularly in Beta-binomial system, $|\candidateset{\dataobs}| = n + 1$, then it is enough to set $\gamma$ as $ \leq \ln(1 - \frac{\epsilon}{2 \ln (\frac{\delta'}{2 |n + 1|})})$.
...
\begin{equation*}
\hexpmechPr{\dataobs}{z = r} \leq e^{\epsilon} \underset{z \thicksim \hexpmech(\dataobs')}{Pr}[z = r] + \delta. 
\end{equation*}


	Given that $\hexpmechPr{\dataobs}{z = r} = \hexpmechPr{\dataobs}{u(\dataobs,z) = u(\dataobs,r)}$, let $U = u(r, \dataobs)$, $U_1 = u(z, \dataobs') - u(z, \dataobs$), $U_2 = U + U_1$ and $U_3 = U_2 \cdot \frac{S(\dataobs)}{S(\dataobs')} \cdot \ln(\frac{NL(\dataobs)}{NL(\dataobs')})$. Then,


	Given that $2\Big( \hexpmechPr{\dataobs}{z = r}\Big) = \hexpmechPr{\dataobs}{u(z, \dataobs) = u(r, \dataobs)}$, let $U = u(\dataobs,r)$, $U_1 = u(z, \dataobs') - u(z, \dataobs)$, $U_2 = U + U_1$ and $U_3 = U_2 \cdot \frac{S(\dataobs)}{S(\dataobs')} \cdot \ln(\frac{NL(\dataobs)}{NL(\dataobs')})$. Then,
