...
$$
\begin{array}{rcl}
& Pr\big[\hlg(\bysinfer(\dataobs), r_L ) 
= \hlg(\betad(\alpha, \beta), \betad(\alpha + t, \beta - t)\big] 
=  \frac{1}{2} (e^{- \frac{\epsilon (t - 1)}{\Delta \bysinfer}} - e^{- \frac{\epsilon (t)}{\Delta \bysinfer}})\\
\end{array}
$$
where $r_L = \betad(\alpha + \lfloor Y \rfloor, \beta - \lfloor Y \rfloor)$.\\
By simplification the Hellinger distance in $\hlg(\betad(\alpha, \beta), \betad(\alpha + t, \beta - t)$, we get:

\begin{itemize}
  \item[\textbf{case 1:}] \textbf{t is even}\\
  \begin{align*}
  & Pr\Bigg[\hlg(\bysinfer(\dataobs), r_L ) 
