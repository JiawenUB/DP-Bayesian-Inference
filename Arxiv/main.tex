...
\begin{lem}
\label{lem_dilation}
Given $\hexpmech(\dataobs)$ in Beta-binomial system, for any $r\in \candidateset$, let $\lambda < |\gamma|$, $\epsilon$, $|\delta'| < 1$ and $\gamma \leq \ln(1 - \frac{\epsilon}{2 \ln (\frac{\delta'}{2 |\candidateset|})})$, the following \emph{dilation property} holds:
\begin{equation*}
\hexpmechPr{\dataobs}{\ux{z} = \ux{r}}
\leq
  ...
  \end{proof}
      ...
      \begin{itemize}
        ...
        \item {\boldmath $\lambda > 0$}:
          ...
          Particularly, $|\candidateset| = n + 1$ because in Beta-binomial system.
...
\subsubsection{$\hexpmech$ $2\epsilon$ Privacy Analysis}
