...
Accuracy bound for Laplace mechanism is provided by its probability density function\cite{dwork2014algorithmic}:
$$
Pr[|Y| \geq t \cdot b] = e^{- t},
$$
where $Y \thicksim Lap(b)$, i.e.,
\begin{equation*}
Pr[|Y| \geq t] = e^{- \frac{t \epsilon}{\Delta \bysinfer}}
\end{equation*}
in our case, where $Y \thicksim Lap(\frac{\Delta \bysinfer}{\epsilon})$.

After post-processing, Laplace noise is discretized. Then the probability mess function for discretized Laplace mechanism is obtained by:
\[
...
Let $\betad(\alpha, \beta)$ be the true posterior distribution, i.e., $\bysinfer(\dataobs) = \betad(\alpha, \beta)$, and $r_L$ be the posterior produced by Laplace mechanism. By applying the probability mess function of discretized Laplace mechanism in our case with Hellinger distance, we get:
$$
\begin{array}{rcl}
...
\end{array}
$$
where $r_L = \betad(\alpha + \lfloor Y \rfloor, \beta - \lfloor Y \rfloor)$.\\
By simplification the Hellinger distance in $\hellinger(\betad(\alpha, \beta), \betad(\alpha + t, \beta - t)$, we get:
...
\begin{itemize}
  ...
  = \quad & \frac{1}{2} (e^{- \frac{\epsilon (t - 1)}{\Delta \bysinfer}} - e^{- \frac{\epsilon (t)}{\Delta \bysinfer}})\\
  \end{align*}
...
We explored following accuracy bound for $\hexpmech$:
  \begin{align*}
  \hexpmechPr{\dataobs}{\hellinger(\bysinfer(\dataobs), z) \geq c} 
  = & \sum\limits_{\{r | \hellinger(\bysinfer(\dataobs), r) \geq c\}} \frac{\exp{\frac{- \epsilon \hellinger(\bysinfer(\dataobs), r)}{4 S(\dataobs)}}}{\unomalizer{\dataobs}}\\
  \leq & \frac{|\candidateset| \exp{(\frac{- \epsilon c}{4 S(\dataobs)}})}{\unomalizer{\dataobs}}\\
  \leq & \frac{|\candidateset| \exp{(\frac{- \epsilon c}{4 S(\dataobs)}})}{|\candidateset|\exp{(\frac{- \epsilon}{4 S(\dataobs)})}}\\
  = & \exp{\big(\frac{ \epsilon (1 - c)}{4 S(\dataobs)}}\big)\\
  \end{align*}

Moreover, based on the accuracy bound in Sec. \ref{subsec_accuracy_global}, we also derived a loose bound:
...
Unfolding the Hellinger distance in the accuracy bound, we get:
...
\begin{center}
\centering
    \includegraphics[width=0.6\textwidth]{privacyloss.eps}
\caption{Concrete privacy calculation under settings that: prior distribution:$[1,1]$, $\epsilon = 1.0$ and observed data are uniformly distributed}
\label{fig_privacy}
\end{center}
\end{figure}
