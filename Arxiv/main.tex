...
Accuracy bound for Laplace mechanism is given by its probability density function\cite{dwork2014algorithmic}:
$$
...
Let $\betad(\alpha, \beta)$ be the true posterior distribution, i.e., $\bysinfer(\dataobs) = \betad(\alpha, \beta)$, and $r_L$ be the posterior produced by Laplace mechanism, i.e., $r_L = \betad(\alpha + \lfloor Y \rfloor, \beta - \lfloor Y \rfloor)$. By applying the probability mess function of discretized Laplace mechanism in our case with Hellinger distance, we get:
$$
\begin{array}{rcl}
...
%   \end{align*}
 ...
 \begin{itemize}
  ...
  \begin{align*}
  &\hexpmechPr{\dataobs}{\hellinger(\bysinfer(\dataobs), z)^2 =  1 - \prod_{k = 0}^{\frac{t}{2} - 1}
  \sqrt{1 - \frac{\frac{t}{2}}{a + k + \frac{t}{2}}}
  \cdot
  \prod_{k = 1}^{\frac{t}{2}}
  ...
  \cdot
  \prod_{k = 1}^{\frac{t}{2}}
  \sqrt{1 - \frac{\frac{t}{2}}{\beta - k}}}}{4 S(\dataobs)})}}{\unomalizer{\dataobs}}
  \end{align*}
