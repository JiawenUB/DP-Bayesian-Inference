...
\end{itemize}
In all cases above, probabilities are fixed whenever data size changes.
...
\hline
 ...
 \hline
 $\hexpmechPr{\dataobs}{\hellinger(\bysinfer(\dataobs), z) = c}$
 & $\frac{1}{\nomalizer{\dataobs}}$ & $\frac{\exp{(\frac{- \epsilon \sqrt{1 - \frac{\Gamma(\alpha+\frac{1}{2})}{\Gamma(\alpha)} \cdot
...
\hline
 ...
 \hline
 $\hexpmechPr{\dataobs}{\hellinger(\bysinfer(\dataobs), z) = c}$
 & $\frac{2}{\nomalizer{\dataobs}}$ & $\frac{2 \exp{(\frac{- \epsilon \sqrt{1 - \frac{\Gamma(\alpha+\frac{1}{2})}{\Gamma(\alpha)} \cdot
  \frac{\Gamma(\beta - \frac{1}{2})}{\Gamma(\beta)}}}{4 S(\dataobs)})}}{\nomalizer{\dataobs}}$ & \\
 \hline
\end{tabular}
\caption{Accuracy Comparison in Theoretical}
...
The probability of getting the true posterior, or posterior with certain step from Laplace mechanism is fixed whatever the data size or prior changes. 
...
\centering
  \subfigure[2 dimensions of data size 1000]{
    \includegraphics[width=0.45\textwidth]{epsilon5_2d_1000.eps}
  \label{subfig_theory_2d}
  }
  \subfigure[2 dimensions of data size 10000]{
    \includegraphics[width=0.45\textwidth]{epsilon5_2d_10000.eps}
  \label{subfig_theory_3d}
  }
