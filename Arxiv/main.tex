...
\end{equation*}
where $Y \thicksim Lap(\frac{\Delta \bysinfer}{\epsilon})$.

After post-processing, Laplace noise is discretized. Then the accuracy bound for Laplace mechanism is obtained by:
\[
Pr[\lfloor Y \rfloor = t] 
= Pr[ t-1 \leq Y < t] 
= \frac{1}{2} (e^{- \frac{\epsilon (t - 1)}{\Delta \bysinfer}} - e^{- \frac{\epsilon (t)}{\Delta \bysinfer}}).
\]

Let $\betad(\alpha, \beta)$ be the true posterior distribution, i.e., $\bysinfer(\dataobs) = \betad(\alpha, \beta)$, and $r_L$ be the posterior produced by Laplace mechanism. By applying the accuracy bound into Hellinger distance in our case:
$$
\begin{array}{rcl}
& Pr[\hlg(\bysinfer(\dataobs), r_L ) = \hlg(\betad(\alpha, \beta), \betad(\alpha + t, \beta - t)] 
=  \frac{1}{2} (e^{- \frac{\epsilon (t - 1)}{\Delta \bysinfer}} - e^{- \frac{\epsilon (t)}{\Delta \bysinfer}})\\
\end{array}
$$
...
$$
Pr[\hlg(\bysinfer(\dataobs), r_L ) 
= 1 - \prod_{k = 0}^{\frac{t}{2} - 1}
\sqrt{1 - \frac{\frac{t}{2}}{a + k + \frac{t}{2}}}
\cdot
\prod_{k = 1}^{\frac{t}{2}}
\sqrt{1 - \frac{\frac{t}{2}}{\beta - k}}] =  \frac{1}{2} (e^{- \frac{\epsilon (t - 1)}{\Delta \bysinfer}} - e^{- \frac{\epsilon (t)}{\Delta \bysinfer}})
$$
