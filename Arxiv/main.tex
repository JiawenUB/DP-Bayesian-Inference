...
\begin{thm}
 ...
 In Dirichlet-multinomial system, for 3 different Dirichlet distributions $\dirichlet(\valpha)$, $\dirichlet(\vbeta_1)$ and $\dirichlet(\vbeta_2)$, $\hellinger(\dirichlet(\valpha),\dirichlet(\vbeta_1)) = \hellinger(\dirichlet(\valpha),\dirichlet(\vbeta_2))$ iff:
  ...
  $$
  where $set(\cdot)$ means we only consider the value of each dimension in the vector without the order.
 \end{thm}
...
\begin{thm}
 ...
 $$ 
 and $|z| $ can have $\frac{k! k!}{|a_1|! |a_2|! \cdots |a_{k_a}|! \cdot |b_1|! |b_2|! \cdots |b_{k_b}|!}$ possible values, where $\sum{|a_i|} = \sum{|b_i|} = k$.
 \end{thm}

 \begin{proof} of Theorem \ref{thm_candidate_divide_dirichlet}.

 Let $\bysinfer(\dataobs) = \dirichlet(\valpha)$, and $\dirichlet(\vbeta)$ be a candidate from $\candidateset$, 
 $a_i$ is the distinct value in $\valpha$ and $b_i$ the same in $\vbeta$, 
 $|a_i|$ denotes the number of values in $\valpha$ having the value $a_i$ and $|b_i|$ the same in $\vbeta$.
 Then, by combination and permutation rules, there are $\frac{k! k!}{|a_1|! |a_2|! \cdots |a_{k_a}|! \cdot |b_1|! |b_2|! \cdots |b_{k_b}|!}$ possible combination that satisfy the condition in Theorem \ref{thm_hellinegr_equality_dirichlet}.
 \end{proof}
