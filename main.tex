...
In the Bayesian inference, first there is a prior distribution $\pi(\alphas)$ to present our belief about parameter $\alphas$. Then, we get some observed data $x$ sizing $n$, and produce a posterior distribution $Pr(\alphas | x)$. The Bayesian inference is based on the Bayes' rule to calculate the posterior distribution:
\begin{equation*}
Pr(\alphas | x) = \frac{Pr(x | \alphas) \pi(\alphas)}{Pr(x)}
\end{equation*}
It is denoted as $\bysinfer(x,\pi(\alphas))$ taking an observed data set $x \in \mathcal{X}^n$ and a prior distribution $\pi(\alphas)$ as input, outputting a posterior distribution of the parameter $\alphas$. For conciseness, when prior is given, we use $\bysinfer(x)$. $n$ is the size of the observed data size.

In our inference algorithm, we take a Dirichlet distribution as prior belief for the parameters, $\dirichlet(\alphas)$, and the Bernoulli distribution as the statistic model for $Pr(x | \alphas)$.  

Columns of the data set is corresponding to the number of parameters ($m$) in Dirichlet distribution. The posterior distribution is still a Dirichlet distribution with parameters $(\alpha_1 + n_1, \alpha_2 + n_2, \cdots, \alpha_m + n_m)$, where $n_i$ is the number of $1$ at the column $i$ in observed data set.

In the case that $m = 2$, it is reduced to a Beta distribution $\betad(\alpha, \beta)$. We can have only one column take value from $\{0,1\}$. The posterior is computed as $(\alpha + n_1, \beta + n_0)$, where $n_1$ is the number of $1$ in the data set and $n_0$ is the number of $0$ in the data set.
...
\begin{center}
\centering
  \subfigure[Data size $n = 200$ with global $\epsilon = 0.5$]{
...
We study the accuracy property of the exponential mechanism with three kind of sensitivity, as well as the Laplace mechanism for comparison. As in Fig. \ref{fig_data}, the two figures shown the accuracy of four algorithms under same configuration. The accuracy is measured by Hellinger distance between output $r^*$ and the true inference result $\bysinfer(x)$, $\hlg(r^*, \bysinfer(x))$.
