...
\quad 		& \Delta_l u(x) = |\hlg(\bysinfer(x), r) - \hlg(\bysinfer(y'), r)|,\\
					& \max_{\{|x,y'| \leq 1;y'\in \mathcal{X}^n\}}\max_{\{r\in R\}};\\
...
\quad 	& \Delta_l u(x) = \hlg(\bysinfer(x), \bysinfer(y')),\\
...
\begin{equation*}
\underset{z \thicksim \expomech}{Pr}[u(r,x) = \hat{s}] \leq e^{\frac{\epsilon}{2}} \underset{z \thicksim \expomech}{Pr}[u(r,x) = (\Delta + \hat{s})] + \frac{\delta}{2}
\end{equation*}
