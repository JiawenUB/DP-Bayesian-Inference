...
\begin{equation*}
\underset{r \thicksim \hexpmech(x)}{Pr}[u(r) = z]
\leq
e^{\frac{\epsilon}{2}} \underset{r \thicksim \hexpmech(x)}{Pr}[u(r) = e^{\lambda} z] + \frac{\delta}{2},
\end{equation*}
...
We set the $h(z) = Pr[u(\hexpmech(x)) = z] = \frac{exp(\frac{\epsilon z}{2 S(x)})}{NL_x}$.
