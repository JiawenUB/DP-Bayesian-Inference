...
\item  Baseline approach: 
  ...
  \end{itemize}
  \end{itemize}
  {\color{red}{But $\Delta \bysinfer$ grows linearly with the dimension: too noisy when we generalize to Dirichlet-Multinomial ({\small$\dirichlet(\cdot)$}) model}}.
\end{column}
...
\subfigure[2 dimensions with data size $100$]{
  ...
  \subfigure[\footnotesize{3 dimensions, data size $\in [100,500]$} ]{
    \includegraphics[width=0.31\textwidth]{poster_2.eps}
  \label{subfig_sampling_3d}
  } 
  \subfigure[\footnotesize{4 dimensions, data size $\in [100,600]$}]{
    \includegraphics[width=0.304\textwidth]{poster_3.eps}
