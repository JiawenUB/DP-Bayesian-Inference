...
Two different upper bounds to the value
\begin{align}
  \label{eqn:eq1}
...
The idea of $\lapmech$ is to add Laplacian noise to the differences of the respective numeric parameters of the prior
...
\begin{algorithm}
  ...
  \STATE \quad {\bf let} $\dirichlet{\valpha'} = \inferd{\valpha}{\vtheta}{\dataobs}$
...
\begin{lem}
  ...
  \begin{proof} By noticing that $\mathcal{X}-1$ is an upper bound to \ref{eqn:eq1}. \end{proof}
\end{lem}


\subsection{Improving the sensitivity ($\ilapmech$)}
\label{sec_ell1_hist}
By seeing the parameters of the distributions as histograms, we can give a tighter upper bound to \ref{eqn:eq1}.
Indeed  $\lapmech$ can be reduced to a constant. Algorithm $\ilapmech$ in Algorithm \ref{mech:ilapmech} takes advantage of this fact. 
Indeed, when $\mathcal{X}=2$, \ref{eqn:eq1} is upper bounded by 1, while when $\mathcal{X}=2$ \ref{eqn:eq1} is upper bounded by 2.
In Algorithm \ref{mech:ilapmech} the algorithm $\ilapmech$ is presented.
\begin{algorithm}
