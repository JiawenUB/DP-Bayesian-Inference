...
the Dirichlet-Multinomial model).
  ...
  % \RETURN $a_{k+1}$.
  \noindent {\bf return} $\betad(\tilde{\alpha}, \tilde{\beta})$
  \end{algorithmic}
  \end{algorithm}
...
\subsection{Improving the sensitivity ($\ilapmech$)}
 ...
 write $\ilapmech(\dataobs)$ when the other parameters are obvious from the context.
  ...
  \STATE {\bf return} $\dirichlet{\tilde{\valpha}}$
  \end{algorithmic}
  \end{algorithm}
...
% <<<<<<< HEAD
% \subsubsection{Exponential Mechanism with Hellinger Distance Metric ($\expmech$)}
% Our exploration starts from the standard exponential mechanism $\expmech(\dataobs,u,\candidateset)$, which samples an element from the candidate set $\candidateset$ with probability proportional to $\exp(\frac{\epsilon u(\dataobs,r)}{2 GS})$:
% \[
% \underset{z \thicksim \expmech(\dataobs,u,\candidateset) }{\Pr}[z=r] = \frac
% {\exp(\frac{- \epsilon u(\dataobs,r)}{2 GS})}
% {\Sigma_{r' \in \mathcal{R}}\exp(\frac{\epsilon u(\dataobs,r')}{2 GS})},
% \]
% where $u(\dataobs,r)$ is the Hellinger scoring function over candidates, $\hellinger(\bysinfer(\dataobs), r)$, and $GS$ is the global sensitivity of the scoring function (i.e. the global sensitivity of the Bayesian inference w.r.t the Hellinger distance), calculated by:
% =======
\subsubsection{A first approach: the $\expmech$ mechanism}
$\expmech(x,u,\candidateset)$  outputs an element $r\in\candidateset$ with  probability: % proportional to $\exp(\frac{\epsilon u(x,r)}{2 GS})$.
\[
%\underset{z \thicksim \expmech(x,u,\candidateset) }{\Pr}[z=r] = \frac
\frac
  {\exp(\frac{\epsilon\cdot u(x,r)}{2 GS})}
{\Sigma_{r' \in \mathcal{R}}\exp(\frac{\epsilon\cdot u(x,r')}{2\cdot GS})},
\]
...
Lemma \ref{??} gives us a finite bound on $GS$, while lemma \ref{??} proves privacy $\epsilon$ differential privacy of $\expmech$.


\subsubsection{A second approach based on local sensitivity: $\lexpmech$}
\label{subsec_emls}

Figure \ref{??} show that  the accuracy of $\expmech$ is poor if compared with the one of $\ilapmech$. This is due to the fact that
...
\frac
  {\exp(\frac{\epsilon \cdot u(x,r)}{2\cdot LS(\dataobs)})}
{\displaystyle\sum_{r' \in \mathcal{R}}\exp(\frac{\epsilon\cdot u(x,r')}{2\cdot LS(\dataobs)})},
\]
...
\]
\end{lem}
The interested reader can check the proof in the Appendix \ref{??}.
...
\end{thm}
The interested reader will find the proof in Appendix \ref{??}.
...
\[
  ...
  {\displaystyle\sum_{r' \in \mathcal{R}}\exp(\frac{\epsilon\cdot u(x,r')}{4\cdot S(\dataobs)})},
\]
