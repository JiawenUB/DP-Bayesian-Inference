...
\todo{What is the value of $\varepsilon$?}
%\begin{align*}
%&\lapmechPr{\dataobs}{
...
\[
\ilapmechPr{\dataobs}{\begin{aligned}
\lefteqn{\hellinger(\bysinfer(\dataobs), r_L )}\\ 
&= \hellinger(\betad(\alpha, \beta), \betad(\alpha + t, \beta - t)
\end{aligned}
}?
...
%\]
By the triangle inequality of Hellinger distance,
the local sensitivity  (\ref{eq:local_sensitivity_scoring}) presented in Sec. \ref{sec_mechs} can be simplified to
\[
LS(\dataobs) = \max\limits_{\dataobs' \in \datauni^n:\adj{\dataobs}{\dataobs'}} \hellinger(\bysinfer(\dataobs), \bysinfer(\dataobs')).
\]
\todo{
Do we need the experimental on this formula?
It is proved mathematically by case analysis.
For any $r \in \mathcal{R}$, we have
\begin{align*}
&\lvert \hellinger(\bysinfer(\dataobs), r) - \hellinger(\bysinfer(\dataobs'), r)\rvert\\
& = 
\begin{cases}
\hellinger(\bysinfer(\dataobs), r) - \hellinger(\bysinfer(\dataobs'),r) & \text{positive}\\
\hellinger(\bysinfer(\dataobs') - \hellinger(\bysinfer(\dataobs), r)& \text{negative}
\end{cases}\\
& \leq 
\begin{cases}
...
&= \hellinger(\bysinfer(\dataobs'), \bysinfer(\dataobs))
\end{align*}
I think it would be a proposition.
}
