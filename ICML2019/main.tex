...
\]
with $\Delta \alpha_i=\sum_{j=1}^{n}\iverson{x_j=a_i}$, where $\iverson{\cdot}$ represents Iverson bracket notation.
...
%\]
By the triangle inequality of Hellinger distance, the local sensitivity  (\ref{eq:local_sensitivity_scoring}) presented in Sec. \ref{sec_mechs} can be simplified to $\max\limits_{\dataobs' \in \datauni^n:\adj{\dataobs}{\dataobs'}} \hellinger(\bysinfer(\dataobs), \bysinfer(\dataobs'))$.
