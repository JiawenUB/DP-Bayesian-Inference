...
Section \ref{sec_ell1_dim} presents a first mechanism called $\lapmech$ based on the Laplace primitive
where the sensitivity is measured w.r.t. to $\ell_1$ norm.
We will then notice, Section \ref{??}, that this mechanism's accuracy can be improved by performing a better analysis on
the sensitivity of the inference process.
Section \ref{???}, shows an instance of the exponential mechanism where the scoring
