...
\cite{wang2015privacy} proved the differentially privacy ofsampling from
...
Laplace mechanism is mostly used while exponential mechanisms have not yet been fully explored. Also, $\ell_1$ norm is mainly used to measure the accuracy while \emph{probabilistic distances} are more suitable when the output of a differentially private mechanism is a probability distribution.

In this work, we explore the design space for differentially private bayesian inference mechanism, by considering different metrics and different algorithms. 
...
We explore three main mechanisms based on the Laplace algorithm family and exponential algorithm family. Also, we apply different metric them by calibrating noise to the sensitivity of that utility function, where the utility function is the probability metric we want to
