...
\begin{itemize}
    ...
    \item A full proof of $(\epsilon, \delta)$-differential privacy of the mechanism is given in the paper. \gp{it's only eps-d.p for now}
...
% 
  ...
  \STATE \quad {\bf Initialize} associative map $pr$ of size $|\candidateset|$.
  \STATE \quad {\bf For} $r$ in $\candidateset$:
  \STATE \quad \quad $pr[r] = 
  {\exp(\frac{\epsilon\cdot \ux{r}}{2\cdot GS})}
/ {\Sigma_{r' \in \mathcal{R}}\exp(\frac{\epsilon\cdot u(\dataobs, r')}{2\cdot GS})}$
\STATE \quad {\bf sample} in $z$ from probability distribution $pr$.
\STATE {\bf return} $z$
  \end{algorithmic}
  \end{algorithm}
\begin{lem}
...
where $S(\dataobs)$ is the 1-smooth sensitivity of  $\hellinger(\bysinfer{\dataobs},\cdot)$. {\color{red}{why do we choose 1???}}
  ...
  \begin{algorithmic}
  \INPUT observed data set $\dataobs \in \mathcal{X}^n$,  prior: $\dirichlet{\valpha}$, $\epsilon$
  \STATE \quad {\bf let} $\dirichlet{\valpha'} = \bys{\valpha}{\dataobs}$.
  \STATE \quad {\bf Initialize} associative map $pr$ of size $|\candidateset|$.
  \STATE \quad {\bf For} $r$ in $\candidateset$:
  \STATE \quad \quad $pr[r] = 
