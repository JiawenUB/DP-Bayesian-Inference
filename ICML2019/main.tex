...
\subsubsection{Exponential Mechanism with Hellinger Metric and Local Sensitivity ($\lexpmech$)}
\label{subsec_emls}
In order to achieve good accuracy, we make an attempt on the Exponential mechanism with local sensitivity $\lexpmech(\dataobs,u,\candidateset)$. This is just an attempt since local sensitivity is non-private. $\lexpmech$ shares the same candidate set and utility function as $\expmech$, outputting a result $r \in \mathcal{R}$ with probability proportional to $\exp(\frac{- \epsilon u(\dataobs,r)}{2 LS(\dataobs)})$:
\[
\underset{z \thicksim \lexpmech(x,u,\candidateset)}{\Pr}[z=r] = \frac
{\exp(\frac{- \epsilon u(\dataobs,r)}{2 LS(\dataobs)})}
{\Sigma_{r' \in \mathcal{R}}\exp(\frac{\epsilon u(\dataobs,r')}{2 LS(\dataobs)})},
\]
...
\subsubsection{Exponential Mechanism with Hellinger Metric and Smoothed Sensitivity ($\hexpmech$)}
...
\begin{lem}
$\lapmech$ and $\ilapmech$ are $\epsilon-$differentially private by \cite{dwork2014algorithmic}.
\end{lem}
...
\begin{lem}
$\lexpmech$ is non-differentially private.
\end{lem}
