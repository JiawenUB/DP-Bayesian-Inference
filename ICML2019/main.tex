...
allows to derive a \emph{posterior} distribution,
...
\cite{wang2015privacy} proved the differential privacy of sampling from
the posterior distribution under specific conditions, \cite{zhang2016differential} and \cite{foulds2016theory}
designed differentially private (DP) mechanisms for this inference process. 
...
Laplace mechanism is mostly used while exponential mechanisms have not yet been fully explored. Also, $\ell_1$ norm is mainly used to measure the accuracy while \emph{probabilistic distances} are more suitable when the output of a DP mechanism is a probability distribution.

In this work, we explore the design space for DP bayesian inference, by considering different metrics and different algorithms. 
We focused on two discrete models, the Beta-Binomial and the
Dirichlet-multinomial models, and one distribution distance, Hellinger distance. 
We design mechanisms based on the Laplace algorithm and exponential algorithm and adopt different metrics in them by calibrating noise to the sensitivity of metric that we want to
use to measure accuracy. To show the usefulness of these mechanisms we
show experimental analysis on their performance.
\end{abstract}
...
is given by a Dirichelet distribution, $\dirichlet{n}{\valpha}$, for $k\in\mathbb{N}$,
...
A randomized mechanism $\mathcal{M}$ with domain $\mathbb{N}^{|\mathcal{X}|}$ and range $\range$, is $(\epsilon, \delta)$-differential privacy, if for all adjacent
\footnote{Given $\dataobs, \dataobs'\in\{0,1\}^n$  we say that $\dataobs$ and $\dataobs'$ are adjacent and we write, $\adj{\dataobs}{\dataobs'}$, iff\\
...
This gives us motivation on exploring the design space
of mechanisms by considering different norms (a distribution metric)
