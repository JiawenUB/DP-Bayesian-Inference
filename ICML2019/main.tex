...
By applying the result of \cite{dwork2014algorithmic},
we obtain $\epsilon-$differential privacy of the mechanism $\expmech$.
\begin{lem}
\label{lem_exp_private}
$\expmech$ is $\epsilon-$differentially private.
\end{lem}
...
In this paper, we prove the $\epsilon-$differential privacy of the 
exponential mechanism $\hexpmech$ with $\gamma$-smooth sensitivity.
\begin{lem}
\label{lem_hexpmech_privacy}
$\hexpmech$ is $\epsilon$-differential private when $\gamma = 1.0$.
\end{lem}
\todo{(Tetsuya) I think this should be theorem or proposition.}
...
\]
%where $\betad(\alpha, \beta)$ is the true posterior distribution
...
The accuracy bound of $\lapmech$ is given by
\begin{align*}
&
\lapmechPr{\dataobs}{
%\begin{aligned}
...
%  $c = \hellinger(\betad(\alpha, \beta), \betad(\alpha + t, \beta - t)$
  ...
  &  = \frac{\exp{(\frac{- \epsilon c(t)}{4 S(\dataobs)})}}{\nomalizer{\dataobs}}%\\
