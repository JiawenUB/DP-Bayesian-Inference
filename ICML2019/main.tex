...
We explore the design space for differentially private Bayesian
inference.  We focused on two discrete models for parametric Bayesian
inference i.e., the Beta-Binomial and the Dirichlet-multinomial
models. We propose a variation of the exponential mechanism that uses
a scoring function based on a metric over probailistic measures, i.e.,
the Hellinger distance. We compare it with other approaches based on
the perturbation of the parameters of the posterior distribution.  We
measure the accuracy using the Hellinger distance over the posterior
distribution released and the real one as opposed to measuring the
$\ell_1$ or $\ell_{\infty}$ distance on the parameters.  We finally
present an experimental analysis on the accuracy of these mechanisms.
\end{abstract}
...
\todo{This
accuracy metric is meaningless when the results released are
distributions rather than numerical values. In contrast, distribution
metrics such as $f$-divergence, Hellinger distance, etc. come into
mind overtly when we are measuring distance between
distributions. }
{\color{blue}im not sure this is a good way to put it...}
...
\subsubsection{Using $\ell_1$ Norm Metric}
  We firstly present the fully explored mechanism in Laplace family -- Laplace mechanism. This mechanism ($\lapmech(\dataobs,\bysinfer, \mathsf{prior}, \mathsf{likelihood}, \epsilon)$) simply adds Laplace noise with post-processing to the posterior distribution parameters, producing an output: $\betad(\alpha +  \lfloor{\Delta \alpha + \eta}\rfloor^n_0,\beta + n - \lfloor{\Delta \alpha + \eta}\rfloor^n_0)$ with $\eta \thicksim \lap{0}{\frac{\Delta \bysinfer}{\epsilon}}$, where $\lfloor \cdot \rfloor^n_0$ is taking the floor value and truncating into $[0,n]$ to make sure the noised posterior is valid, as shown in Mechanism \ref{mech:lapmech}.\\
  ...
  In Dirichlet-multinomial model, $\lapmech$ produces output: $ \dirichlet{\alpha_1 +  \lfloor{\Delta \alpha_1 + \eta_1}\rfloor^n_0, \cdots, \alpha_k + \lfloor n - \sum_{i = 1}^{k-1}\lfloor{\Delta \alpha_i + \eta_i}\rfloor^n_0 \rfloor^n_0},$
...
\subsubsection{Using $\ell_1$ Norm with Improved Sensitivity}
 The $\lapmech$ presented above can be improved in our Bayesian inference model, by adding noise scaled to smaller sensitivity in this improved Laplace mechanism $\ilapmech$.\\
 In terms of two adjacent data sets $\dataobs, \dataobs'$, their posterior distributions by Bayesian inference -- $\bysinfer(\dataobs), \bysinfer(\dataobs')$ -- which parameter differs at most in 2 dimensions even though extended to Dirichlet-multinomial mode, i.e., $\Delta \bysinfer \leq 2$. As a results, it is enough to use sensitivity $1$ in 2 dimensions and $2$ in higher dimensions.\\
...
with $\eta_i \thicksim \lap{0}{\frac{2}{\epsilon}}$ in Dirichlet-multinomial model. Details are shown in Mechanism \ref{mech:ilapmech}.
  ...
  \STATE \quad {\bf let} $\dirichlet{\valpha'} = \bysinfer(\multinomial{n}{\vtheta}, \dirichlet{\valpha}, \dataobs)$.
  \STATE \quad {\bf Initialize} $\tilde{\valpha} = (0,\dots )$, $k = |\mathcal{X}|$.
...
Exponential mechanism with local sensitivity $\lexpmech(x,u,\candidateset)$ share the same candidate set and utility function as it with standard exponential mechanism. This outputs a candidate $r \in \mathcal{R}$ with probability proportional to $exp(\frac{\epsilon u(x,r)}{2 LS(\dataobs)})$:
\begin{equation*}
