...
% Two groups of experimental results both with unit prior $\betad(1,1), \betad(1,1,1)$ and $\betad(1,1,1,1)$, balanced datasets and parameters $\epsilon = 1.0$ and $\delta = 10^{-8}$.
  ...
  In the baseline approach (LapMech), it is enough to add noise with sensitivity $1$ in 2 dimensions and $2$ in higher dimensions since it's equivalent to histogram
...
\end{center}
\end{figure}
\footnotesize{Experiments above are with unit prior $\betad(1,1), \betad(1,1,1)$ and $\betad(1,1,1,1)$, balanced datasets, $\epsilon = 1.0$ and $\delta = 10^{-8}$.}
\end{block}
