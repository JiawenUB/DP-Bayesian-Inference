...
\begin{block}{Differentially Private Bayesian Inference and Motivation}
\begin{columns} % Subdivide the first main column
\begin{column}{.69\textwidth}
In a baseline approach, we release a private version of posterior $\betad (\tilde\alpha,\tilde\beta)=(\alpha +  \widetilde{\Delta \alpha},\beta + n - \widetilde{\Delta \alpha})$ by sampling noise from $Lap(\mu,\nu)$ with mean $\mu$ and scale $\nu$, i.e., $\widetilde{\Delta \alpha}\sim Lap(\Delta \alpha, \frac{2}{\epsilon})$. This gives us motivation of following two contributions:
\begin{enumerate}
  \item Calibrating the noise w.r.t sensitivity of the accuracy metric (Hellinger distance), v.s. the $\ell_1$ norm in baseline approach.
  \item Applying smooth sensitivity in mechanism v.s. global sensitivity.
        (Global sensitivity of Hellinger distance over $\betad$ distribution takes value at point $(0,0)$ as in Fig. \ref{fig_sensitivity}. But we can improve because it's very smooth and low in central part).
\end{enumerate}
\end{column}
\begin{column}{.28\textwidth}
\begin{figure}[ht]
