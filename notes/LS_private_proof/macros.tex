...
\newcommand{\nomalizer}[1]{\sum_{r'\in \mathcal{R}_{\textup{post}}(#1)} \exp \big(\frac{-\epsilon\cdot \mathcal{H} (\mathsf{BI}(#1),r')}{4 \cdot L(#1)}\big)}

\newcommand{\unomalizer}[1]{\sum_{r'\in \mathcal{R}_{\textup{post}}(#1)} \exp \big(\frac{-\epsilon\cdot u(\textbf{#1}, r')}{4 \cdot L(#1)}\big)}
