...
\newcommand{\nomalizer}[1]{\sum_{r'\in \mathcal{R}_{\textup{post}}} \exp \big(\frac{-\epsilon\cdot \mathcal{H} (\mathsf{BI}(#1),r')}{2 \cdot S(#1)}\big)}

\newcommand{\unomalizer}[1]{\sum_{r'\in \mathcal{R}_{\textup{post}}} \exp \big(\frac{-\epsilon\cdot u(#1, r')}{2 \cdot S(#1)}\big)}
