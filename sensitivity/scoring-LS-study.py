...
class Beta_Distribution(object):
	...
	def __sub__(self, other):
		b = beta_function_naive(self._alpha, self._beta)
		a = beta_function_naive((self._alpha + other._alpha)/2.0, (self._beta + other._beta)/2.0)
		c = beta_function_naive(other._alpha, other._beta)
		# print a, b, c
		# print math.sqrt(b * c)
...
class BayesInferwithBetaPrior(object):
	...
	def _set_observation(self,obv):
		self._observation = numpy.array(obv)
		self._posterior = Beta_Distribution(prior._alpha + numpy.count_nonzero(self._observation == 1),\
			prior._beta + numpy.count_nonzero(self._observation == 0))
...
if __name__ == "__main__":
	...
	sample_size = 4
	epsilon = 0.1
	# prior = Beta_Distribution(2,2)
	# Bayesian_Model = BayesInferwithBetaPrior(prior, sample_size, epsilon)
	# Bayesian_Model._set_observation([1,0,1,0])
	# Bayesian_Model._set_utility()
	# Bayesian_Model._exponentialize_LS()
	# Bayesian_Model._set_observation([0,0,1,0])
	# Bayesian_Model._set_utility()
	# Bayesian_Model._exponentialize_LS()	
	...
	# print Beta_Distribution(80,81) - Beta_Distribution(79, 82)
	# print Beta_Distribution(81,80) - Beta_Distribution(81, 80)
	# print Beta_Distribution(2,2) - Beta_Distribution(3,1)
	# print Beta_Distribution(2,2) - Beta_Distribution(1,3)
	print Beta_Distribution(3,3) - Beta_Distribution(4,2)
	print Beta_Distribution(3,3) - Beta_Distribution(2,4)
	print Beta_Distribution(2,4) - Beta_Distribution(1,5)
